...
export default function ActiveDesactiveCaja({ dataUser }) {
        ...
        try {
            if (userId) {
                ...
                const activarDesactivar = await AxiosConfigsToken({
                    ...
                    url: `/activar_desactivar_caja`
                })

                if (activarDesactivar.data.verificar) {
                    mutate(["obtenerCajass", userId])
                    mutate("obtenerTodasCajasss")
    ...
    React.useEffect(() => {
        setIds(dataUser._id)
        setEtatBool(dataUser.activeAdmin)
    }, [])
        ...
        <>
                ...
                <FormControl >
                    <LoadingButton
                        ...
                        size="small"
                    >
                        <span>{dataUser.activeAdmin? "Desactivar" : "Activar"}</span>
                    </LoadingButton>
