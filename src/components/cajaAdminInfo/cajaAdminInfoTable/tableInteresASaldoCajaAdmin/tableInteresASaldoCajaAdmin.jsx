...
import { GetInteresASaldoCajaAdmin } from './getInteresASaldoCajaAdmin';
...
const VISIBLE_FIELDS = ['nameAdmin', 'Fecha','quantSoldeActual', 'phoneAdmin', 'typeUser', 'cantidad', 'cantidadSaldo', 'nameConfSist', 'phoneConfSist', 'fechaA', 'createdAt', 'Acciones'];
...
function DataTableInteresASaldoCajaAdmin() {
    ...
    const GetRecargasAdmin = async () => {
        ...
        try {
            const res = await AxiosConfigsToken.get(`/get_interes_a_saldo_caja_admin`)
    ...
    //const { data, error, isLoading, } = useSWR("obtenerInteresASaldoCajaAdmin", GetInteresASaldoCajaAdmin, {})
        ...
        <>
            ...
            >
                ...
                <Grid item xs={12} sm={12} md={12} lg={2} xl={2}  >
                    <Box sx={{ width: '100%' }}>
                        <LoadingButton
                            onClick={() => { Buscar('filtrar_interes_a_saldo_caja_admin') }}
                            loading={buscar}
                            variant="contained"
                            color="primary"
                            sx={{ width: "100%" }}
            ...
            <Box >
                <Paper sx={{ display: 'flex', height: '120px', paddingBlock: 1, flexDirection: 'column' }}>
                    <ExcelFile element={<Button sx={{ marginLeft: 1, marginBottom: 1 }} color='inherit' variant='contained' size='small'>Excel</Button>} filename={`interesASaldoAdmin`}>
...
export default DataTableInteresASaldoCajaAdmin