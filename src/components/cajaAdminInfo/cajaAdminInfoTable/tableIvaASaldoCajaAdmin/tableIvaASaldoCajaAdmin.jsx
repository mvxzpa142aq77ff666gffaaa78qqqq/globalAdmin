...
import { GetIvaASaldoCajaAdmin } from './getIvaASaldoCajaAdmin';
...
const VISIBLE_FIELDS = ['nameAdmin', 'Fecha', 'phoneAdmin','quantSoldeActual', 'typeUser', 'cantidad', 'cantidadSaldo', 'nameConfSist', 'phoneConfSist', 'fechaA', 'createdAt', 'Acciones'];
...
function DataTableIvaASaldoCajaAdmin() {
    ...
    //const { data, error, isLoading, } = useSWR("obtenerIvaASaldoCajaAdmin", GetIvaASaldoCajaAdmin, {})
        ...
        <>
            ...
            >
                ...
                <Grid item xs={12} sm={12} md={12} lg={2} xl={2}  >
                    <Box sx={{ width: '100%' }}>
                        ...
                        <LoadingButton
                            onClick={() => { Buscar('filtrar_iva_a_saldo_caja_admin') }}
                            loading={buscar}
                            variant="contained"
                            color="primary"
                            sx={{ width: "100%" }}
            ...
            <Box >
                <Paper sx={{ display: 'flex', height: '120px', paddingBlock: 1, flexDirection: 'column' }}>
                    <ExcelFile element={<Button sx={{ marginLeft: 1, marginBottom: 1 }} color='inherit' variant='contained' size='small'>Excel</Button>} filename={`ivaASaldoAdmin`}>
                    ...
                    <Typography variant='p' sx={{ marginBlock: '3px',fontWeight:"700", marginLeft: 1 }}><span style={{  color: '#212121', fontSize: '13px' }}>Total saldo iva : </span><span style={{ color: "#000000", fontSize: '16px' }}>{Number(totalSaldo).toLocaleString("es-GQ")} XAF</span></Typography>
                </Paper>
            </Box>
        </>
    )
...
export default DataTableIvaASaldoCajaAdmin