...
const VISIBLE_FIELDS = ['nameAdmin', 'phoneAdmin','quantSoldeActual', 'Fecha', 'typeUser', 'quantSolde', 'quantSoldeCount', 'nameConfSist', 'phoneConfSist', 'fechaA', 'createdAt', 'Acciones'];
...
function DataTableRecargasCajaAdmin() {
    ...
    const GetRecargasAdmin = async () => {
        ...
        try {
            const res = await AxiosConfigsToken.get(`/obtener_recargar_caja_admin`)
    ...
    const { data, error, isLoading, } = useSWR("obtenerRecargasCajaAdmin", GetRecargasCajaAdmin, {})
        ...
        <>
            ...
            >
                ...
                <Grid item xs={12} sm={12} md={12} lg={2} xl={2}  >
                    <Box sx={{ width: '100%' }}>
                        ...
                        <LoadingButton
                            onClick={() => { Buscar('filtrar_recargas_caja_admin') }}
                            loading={buscar}
                            variant="contained"
                            color="primary"
                            sx={{ width: "100%" }}
            ...
            <Box >
                <Paper sx={{ display: 'flex', height: '120px', paddingBlock: 1, flexDirection: 'column' }}>
                    <ExcelFile element={<Button sx={{ marginLeft: 1, marginBottom: 1 }} color='inherit' variant='contained' size='small'>Excel</Button>} filename={`recargasAdmin`}>
...
export default DataTableRecargasCajaAdmin