...
export default function InteresASaldoCajaAdmin() {
    ...
    const onSubmit = async (data) => {
        ...
        if (data.cantidad === data.cantidadConfirmacion) {
            ...
            try {
                setLoad(true)
                const sendData = await AxiosConfigsToken({ url: `/intereses_a_saldo_caja_admin`, method: "post", data })
                if (sendData.data.verificar) {
                    toast.success(`${sendData.data.mens}`)
                    mutate('obtenerInteresASaldoCajaAdmin')
                    mutate("obtenerInfoCajaAdmin")
                    reset({
                        cantidad: "",
    ...
    return (
        <Box sx={{ height: "auto", width: '100%', marginBottom: "10px", display: "flex", justifyContent: "end" }}>
            ...
            >
                <Box sx={style}>
                    <Typography variant='h6' sx={{ textAlign: "center", marginBottom: 2, color: "textColorTitle" }}>Formulario para pasar interes a saldo</Typography>
