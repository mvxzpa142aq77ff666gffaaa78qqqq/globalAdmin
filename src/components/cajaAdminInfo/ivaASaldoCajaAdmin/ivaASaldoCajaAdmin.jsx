...
export default function IvaASaldoCajaAdmin() {
    ...
    const onSubmit = async (data) => {
        ...
        if (data.cantidad === data.cantidadConfirmacion) {
            ...
            try {
                setLoad(true)
                const sendData = await AxiosConfigsToken({ url: `/iva_a_saldo_caja_admin`, method: "post", data })
                if (sendData.data.verificar) {
                    toast.success(`${sendData.data.mens}`)
                    mutate('obtenerIvaASaldoCajaAdmin')
    ...
    return (
        <Box sx={{ height: "auto", width: '100%', marginBottom: "10px", display: "flex", justifyContent: "end" }}>
            ...
            >
                <Box sx={style}>
                    <Typography variant='h6' sx={{ textAlign: "center", marginBottom: 2, color: "textColorTitle" }}>Formulario para pasar iva a saldo</Typography>
