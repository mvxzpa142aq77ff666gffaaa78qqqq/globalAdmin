...
export default function FormRecargarCajaMaster({ id }) {
    ...
    const onSubmit = async (data) => {
        ...
        if (data.cantidad === data.cantidadConfirmacion) {
            try {
                setLoad(true)
                const sendData = await AxiosConfigsToken({ url: `/recargar_caja_master`, method: "post", data })
    ...
    return (
        <Box sx={{ height: "auto", width: '100%', marginBottom: "10px", display: "flex", justifyContent: "end" }}>
            ...
            >
                <Box sx={style}>
                    <Typography variant='h6' sx={{ textAlign: "center", marginBottom: 2, color: "textColorTitle" }}>Formulario para recargar caja</Typography>
