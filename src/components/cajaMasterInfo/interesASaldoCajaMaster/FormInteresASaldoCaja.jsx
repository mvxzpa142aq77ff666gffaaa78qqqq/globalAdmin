...
export default function FormInteresASaldoCajaMaster({id}) {
    ...
    const onSubmit = async (data) => {
        ...
        if (data.cantidad === data.cantidadConfirmacion) {
            try {
                setLoad(true)
                const sendData = await AxiosConfigsToken({ url: `/interes_a_saldo_caja`, method: "post", data })
    ...
    return (
        <Box sx={{ height: "auto", width: '100%', marginBottom: "10px", display: "flex", justifyContent: "end" }}>
            <Button
                ...
                onClick={handleOpenM}
                size='medium'
                sx={{ width: "100%",backgroundColor:"#ffffff",marginTop:'12px' }}
            >
                Interes a saldo
            </Button>
            <Modal
            ...
            >
                <Box sx={style}>
                    <Typography variant='h6' sx={{ textAlign: "center", marginBottom: 2, color: "textColorTitle" }}>Pasar interes a saldo</Typography>
