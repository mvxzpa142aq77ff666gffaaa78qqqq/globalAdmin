...
import { GetRecargasTodos, GetRecargasMaster } from './getRecargas'
...
const VISIBLE_FIELDS = ['nameAdmin', 'phoneAdmin', 'typeUser', 'Fecha', 'quantSolde','quantSoldeActual', 'quantSoldeCount', 'nameConfSist', 'phoneConfSist', 'fechaA', 'createdAt', 'Acciones'];
...
const columns1 = [
    ...
    },
    {
        ...
        renderCell: (params) => {
            ...
            const suma = Number(params.row.quantSolde) + Number(params.row.quantSoldeCount)
...
export default function DataTableRecargas() {
    ...
    const GetRecargasMasters = async (userId) => {
        ...
        try {
            const res = await AxiosConfigsToken.get(`/obtener_recargas_id_master/${userId}`)
            console.log(res)
    ...
    const GetRecargasAdmin = async () => {
        setSpinnerArray(true)
        try {
            const res = await AxiosConfigsToken.get(`/todas_las_recargas`)
            if (res.data.data.docs) {
                const datos = res.data.data.docs
                setDataArray(datos)
    ...
    if (arrayMaster.includes(typeUser)) {
        //const { data, error, isLoading, } = useSWR(["obtenerRecargasMasterOOO", userId], () => GetRecargasMaster(userId), {})
            ...
            <>
                ...
                >
                    ...
                    <Grid item xs={12} sm={12} md={12} lg={2} xl={2}  >
                        <Box sx={{ width: '100%' }}>
                            ...
                            <LoadingButton
                                onClick={() => { Buscar('filtrar_recargas_master') }}
                                loading={buscar}
                                variant="contained"
                                color="primary"
                                sx={{ width: "100%" }}
                ...
                <Box >
                    <Paper sx={{ display: 'flex', height: '120px', paddingBlock: 1, flexDirection: 'column' }}>
                        <ExcelFile element={<Button sx={{ marginLeft: 1, marginBottom: 1 }} color='inherit' variant='contained' size='small'>Excel</Button>} filename={`recargas`}>
                            <ExcelSheet data={dataArray} name="transferencia">
                                <ExcelColumn label="Usuario recargado" value="nameAdmin" />
        ...
        //const { data, error, isLoading, } = useSWR(["obtenerRecargasTodosOO", userId], () => GetRecargasTodos(), {})
                ...
                >
                    ...
                    <Grid item xs={12} sm={12} md={12} lg={2} xl={2}  >
                        <Box sx={{ width: '100%' }}>
                            ...
                            <LoadingButton
                                onClick={() => { Buscar('filtrar_recargas_admin') }}
                                loading={buscar}
                                variant="contained"
                                color="primary"
                                sx={{ width: "100%" }}
