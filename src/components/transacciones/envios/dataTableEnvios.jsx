...
import { GetEnviosTodos, GetEnviosMaster } from './getEnvios'
import AppContext from '../../../contexts/ServiceContext';
import { DataGridPremium } from '@mui/x-data-grid-premium';
import { GetEnviosDeUnMaster } from '../../masterInfo/masterInfoTable/tableEnvios/getEnviosMaster';
...
const VISIBLE_FIELDS = ['nameSend', 'phoneSend', 'Fecha', 'adressAdmin', 'verifyRecp', 'iva', 'comision', , 'quantSend', 'interesSocio', 'interesGlobal', 'nameRecep', 'phoneRecep', 'adressRecep', 'nameAdmin', , 'fechaA', 'createdAt', 'phoneAdmin', 'acciones', 'Acciones'];
...
export default function DataTableEnvios() {
    ...
    const columns1 = [
        ...
        arrayMaster.includes(typeUser) ?
            ...
            {
                field: 'interesGlobal',
                headerName: 'interes G-NOB',
                width: 100,
                editable: false,
                renderCell: (params) => {

                    return (Number(params.row.interesGlobal).toLocaleString("es-GQ") + ' ' + 'XAF');
                },
            },
        {
            field: 'interesSocio',
        ...
        arrayMaster.includes(typeUser) ?
            ...
            :
            {
                ...
                renderCell: (params) => {
                    ...
                    return (Number(params.row.iva).toLocaleString("es-GQ") + ' ' + 'XAF');
                },
            },
        {
            field: "verifyRecp",
        ...
        },
        {
            ...
            renderCell: (params) => {
                ...
                return (
                    <>
                        {!verificars ?
                            <FormUpdateEnvio dataUser={currentRow} GetEnviosMaster={GetRecepcionesMasters} GetEnviosAdmin={GetRecepcionesAdmin} />
                            :
                            <></>
                        }
    ...
    const GetRecepcionesMasters = async (userId) => {
        setSpinnerArray(true)
        try {
            const res = await AxiosConfigsToken.get(`/obtener_envios_master/${userId}`)
            console.log(res)
    ...
    const GetRecepcionesAdmin = async () => {
        setSpinnerArray(true)
        try {
            const res = await AxiosConfigsToken.get(`/envios_todos`)
            if (res.data.data.docs) {
                const datos = res.data.data.docs
                setDataArray(datos)
    ...
    if (arrayMaster.includes(typeUser)) {
        //const { data, error, isLoading, } = useSWR(["obtenerEnviosMasters", userId], () => GetEnviosDeUnMaster(userId), {})
        ...
        return (
            <>
                ...
                >
                    ...
                    <Grid item xs={12} sm={12} md={12} lg={2} xl={2}  >
                        <Box sx={{ width: '100%' }}>
                            ...
                            <LoadingButton
                                onClick={() => { Buscar('envios_filtrados_master') }}
                                loading={buscar}
                                variant="contained"
                                color="primary"
                                sx={{ width: "100%" }}
                ...
                <Box >
                    <Paper sx={{ display: 'flex', height: '145px', paddingBlock: 1, flexDirection: 'column' }}>
                        ...
                        <Typography variant='p' sx={{ marginBlock: '3px', marginLeft: 1,fontWeight:"700" }}><span style={{ color: '#212121', fontSize: '13px' }}>Total saldo enviado : </span><span style={{ color: "#000000", fontSize: '16px' }}>{Number(totalSaldo).toLocaleString("es-GQ")} XAF</span></Typography>
                        <Typography variant='p' sx={{ marginBlock: '3px', marginLeft: 1,fontWeight:"700" }}><span style={{ color: '#212121', fontSize: '13px' }}>Total int soicos : </span><span style={{ color: "#000000", fontSize: '16px' }}>{Number(totalIS).toLocaleString("es-GQ")} XAF</span> </Typography>
                    </Paper>
                </Box>
            </>
        )
    } else {
        //const { data, error, isLoading, } = useSWR(["obtenerEnviosTodoss", userId], () => GetEnviosTodos(), {})
        ...
        return (
            <>
                ...
                >
                    ...
                    <Grid item xs={12} sm={12} md={12} lg={2} xl={2}  >
                        <Box sx={{ width: '100%' }}>
                            ...
                            <LoadingButton
                                onClick={() => { Buscar('envios_filtrados_admin') }}
                                loading={buscar}
                                variant="contained"
                                color="primary"
                                sx={{ width: "100%" }}
                ...
                <Box >
                    <Paper sx={{ display: 'flex', height: '244px', paddingBlock: 1, flexDirection: 'column' }}>
                        <ExcelFile element={<Button sx={{ marginLeft: 1, marginBottom: 1 }} color='inherit' variant='contained' size='small'>Excel</Button>} filename={`transferencias`}>
                            <ExcelSheet data={dataArray} name="transferencia">
                                ...
                                <ExcelColumn label="Interes G-NOB" value="interesGlobal" />
                                <ExcelColumn label="Interes Socio" value="interesSocio" />
                        ...
                        <Typography variant='p' sx={{ marginBlock: '3px',fontWeight:"700", marginLeft: 1 }}><span style={{  color: '#212121', fontSize: '13px' }}>Total int. comision :</span><span style={{ color: "#000000", fontSize: '16px' }}>{Number(totalColision).toLocaleString("es-GQ")} XAF</span></Typography>
                        <Typography variant='p' sx={{ marginBlock: '3px',fontWeight:"700", marginLeft: 1 }}><span style={{  color: '#212121', fontSize: '13px' }}>Total int. G-NOB :</span><span style={{ color: "#000000", fontSize: '16px' }}>{Number(totalIG).toLocaleString("es-GQ")} XAF</span></Typography>
                        <Typography variant='p' sx={{ marginBlock: '3px',fontWeight:"700", marginLeft: 1 }}><span style={{  color: '#212121', fontSize: '13px' }}>Total int socios : </span><span style={{ color: "#000000", fontSize: '16px' }}>{Number(totalIS).toLocaleString("es-GQ")} XAF</span> </Typography>
                        <Typography variant='p' sx={{ marginBlock: '3px',fontWeight:"700", marginLeft: 1 }}><span style={{  color: '#212121', fontSize: '13px' }}>Total iva : </span><span style={{ color: "#000000", fontSize: '16px' }}>{Number(totalIva).toLocaleString("es-GQ")} XAF </span></Typography>
                    </Paper>
                </Box>
            </> 
        )
