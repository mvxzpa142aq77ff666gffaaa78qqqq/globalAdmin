...
export default function DataTableEnviosCancelados() {
    ...
    const GetRecepcionesMasters = async (userId) => {
        ...
        try {
            const res = await AxiosConfigsToken.get(`/envios_cancelados_master/${userId}`)
            console.log(res)
    ...
    const GetRecepcionesAdmin = async () => {
        setSpinnerArray(true)
        try {
            const res = await AxiosConfigsToken.get(`/envios_cancelados_todos`)
            if (res.data.data.docs) {
                const datos = res.data.data.docs
                console.log(datos)
                setDataArray(datos)
                ...
                >
                    ...
                    <Grid item xs={12} sm={12} md={12} lg={2} xl={2}  >
                        <Box sx={{ width: '100%' }}>
                            ...
                            <LoadingButton
                                onClick={() => { Buscar('filtrar_envios_cancelados_master') }}
                                loading={buscar}
                                variant="contained"
                                color="primary"
                                sx={{ width: "100%" }}
                ...
                >
                    ...
                    <Grid item xs={12} sm={12} md={12} lg={2} xl={2}  >
                        <Box sx={{ width: '100%' }}>
                            ...
                            <LoadingButton
                                onClick={() => { Buscar('filtrar_envios_cancelados_todos') }}
                                loading={buscar}
                                variant="contained"
                                color="primary"
                                sx={{ width: "100%" }}
