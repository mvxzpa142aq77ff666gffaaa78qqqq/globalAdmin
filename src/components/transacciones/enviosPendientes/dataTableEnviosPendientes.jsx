...
export default function DataTableEnviosPendientes() {
    ...
    const GetRecepcionesMasters = async (userId) => {
        ...
        try {
            const res = await AxiosConfigsToken.get(`/envios_pendientes_master/${userId}`)
            console.log(res)
    ...
    const GetRecepcionesAdmin = async () => {
        setSpinnerArray(true)
        try {
            const res = await AxiosConfigsToken.get(`/envios_pendientes_todos`)
            if (res.data.data.docs) {
                const datos = res.data.data.docs
                console.log(datos)
                setDataArray(datos)
                    ...
                    <>
                        ...
                        >
                            ...
                            <Grid item xs={12} sm={12} md={12} lg={2} xl={2}  >
                                <Box sx={{ width: '100%' }}>
                                    ...
                                    <LoadingButton
                                        onClick={() => { Buscar('filtrar_envios_pendientes_master') }}
                                        loading={buscar}
                                        variant="contained"
                                        color="primary"
                                        sx={{ width: "100%" }}
                        ...
                        <Box >
                            <Paper sx={{ display: 'flex', height: '120px', paddingBlock: 1, flexDirection: 'column' }}>
                                <ExcelFile element={<Button sx={{ marginLeft: 1, marginBottom: 1 }} color='inherit' variant='contained' size='small'>Excel</Button>} filename={`transferencias_cancelada`}>
                                    <ExcelSheet data={dataArray} name="transferencia">
                                        ...
                                        <ExcelColumn label="Enviado en" value="adressSend" />

                                        <ExcelColumn label="Recibido en" value="adressRecep" />
                                        <ExcelColumn label="Cancelado por" value="nameAdminDelete" />
                                        <ExcelColumn label="Motivo" value="description" />
                                ...
                                <Typography variant='p' sx={{ marginBlock: '3px', marginLeft: 1, fontWeight: "700" }}><span style={{ color: '#212121', fontSize: '13px' }}>Total saldo : </span><span style={{ color: "#000000", fontSize: '16px' }}>{Number(totalSaldo).toLocaleString("es-GQ")} XAF</span></Typography>
                            </Paper>
                        </Box>
                    </>
                    :
                        ...
                        >
                            ...
                            <Grid item xs={12} sm={12} md={12} lg={2} xl={2}  >
                                <Box sx={{ width: '100%' }}>
                                    ...
                                    <LoadingButton
                                        onClick={() => { Buscar('filtrar_envios_pendientes_todos') }}
                                        loading={buscar}
                                        variant="contained"
                                        color="primary"
                                        sx={{ width: "100%" }}
